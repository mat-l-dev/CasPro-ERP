...
Email: LOCAL/CI solo adaptador de captura local, sin credenciales Resend ni salida de red al proveedor; nunca correo real, incluso en MANUAL/reenvío o con destinatarios de prueba del proveedor. STAGING captura por defecto y, si se habilita envío, fuerza destinatario seguro permitido para TO/CC/BCC; conserva destinatario solicitado y efectivo, sin fugas por copias. La composición valida entorno y deniega envío ante configuración incompleta/inconsistente; la política de Documents no puede eludirla. Validar esos bloqueos y el dominio remitente/cuenta Resend antes de operación. Las reglas empresariales de entrega viven en [integraciones](../architecture/integrations.md).
...
Drill futuro: restaurar en destino aislado → recrear roles/configuración segura → verificar schema/políticas y pertenencia → comprobar integridad de dinero/stock y versiones/hashes de objetos referenciados → reconciliar cursor de eventos/jobs → registrar duración/pérdida posible. DB y object storage se restauran como recursos separados con cortes compatibles; blob ausente o distinto bloquea disponibilidad/entrega del artefacto, no se fabrica un original. Desactivar efectos externos durante restore/replay: recuperar una outbox no debe volver a enviar correo ni republicar stock ciegamente; conciliar intenciones y resultados externos antes de reactivar.
