...
Decisión posterior del propietario: DeepSeek API / V4.1-Flash inicial, exclusivamente en configuración/adaptador neutral. [Política de coste y programación](../architecture/ai-assistance.md#presupuesto-y-programación-del-proveedor-inicial) posee target10USD/mes, cuotas/reservas, provenance y horario versionado; no se reabre elección de proveedor. [Comparación AF-S24–27](../research/automation-flow-ai-benchmark.md#proveedores-ia-y-evidencia-disponible) contempla OpenAI, Gemini, DeepSeek y modelo local abierto; ninguna popularidad elige proveedor. Accounting declara capacidades necesarias y conjunto de candidatos; Platform configura endpoint/secretos fuera de Git; dueño de política aprueba finalidad/transferencia/retención/coste, no el operador técnico. Modelo local tampoco garantiza calidad ni privacidad sin aislamiento/operación verificados.
...
La cola shadow aplica clases BACKGROUND_ELASTIC/DEADLINE y controles de [programación y cache](../architecture/ai-assistance.md#clases-de-programación-y-cache): un hecho/componente por request inicialmente, lotes de worker limitados, off-peak preferente y reservas antes de dispatch. Sellado, manifest, aislamiento y comparador permanecen; sin locks mientras espera, sin retrasar Accounting oficial ni sugerencia interactiva por tarifa. Microbatch compatible queda LATER tras evaluación; coste/abstención/fallo se atribuyen por unidad sin duplicar consumo.
