...
Estado: visión objetivo y contratos candidatos, no módulos implementados. SPECIFIED significa contenido redactado sujeto al gate global; nunca FROZEN ni aprobación profesional. Los contratos vigentes se localizan por hito en el [índice profundo](../specs/deep-spec-index.md). `Phase` referencia [programa](program.md). `Readiness`: `SPECIFIED`, `ARCHITECTED`, `RESEARCHED`, `CONCEPT`, `BLOCKED`. Cada capability se construye solo en su hito y spec; el mapa no autoriza tablas o carpetas.
...
|---|---|---|---|---|---|
| Identity/session | Identity | credential→principal/session | runtime | acceso; HIGH security | M01 / SPECIFIED CANDIDATE / auth spec / nuevos métodos |
| Legal entity directory | Organization | evidence→entity/context | Identity | separación; HIGH legal | M01 / SPECIFIED CANDIDATE / entity spec / otra entidad |
| Membership/capability | Access | grants→authorized context | Identity, Organization, Audit | deny-by-default; HIGH | M01 / SPECIFIED CANDIDATE / access spec / maker-checker |
| Audit | Audit | actor+transition→append-only event | access primitives | accountability; HIGH | M01 / SPECIFIED CANDIDATE / audit spec / external archive |
...
| Parties | Parties | identity evidence→roles/version | Access, Documents | one counterparty; MEDIUM | M02 / SPECIFIED CANDIDATE / master spec / richer CRM |
| Goods catalog | Catalog | product/SKU/unit/price→version | Access | commerce; MEDIUM | M02 / SPECIFIED / SP2 / variants/serials |
| Purchased service concepts | Procurement | description/period→nonstock line | Parties, Accounting policy later | expenses; MEDIUM tax | M05 / SPECIFIED CANDIDATE / P2P spec / recurring contracts |
| Operational inventory | Inventory | movements→positions/kardex | Catalog | fulfilment; HIGH money/stock | M03 / SPECIFIED CANDIDATE / inventory spec / multiwarehouse |
| Moving average cost | Inventory | entry costs→output cost | operational inventory | margin/COGS source; HIGH accounting | M03 / SPECIFIED CANDIDATE / costing spec / landed costs |
| Physical count | Inventory | plan/count→difference/adjustment | inventory, Audit | integrity; HIGH | M03/M05 / SPECIFIED CANDIDATE / inventory-deep / scanners |
...
| Commercial obligation | Sales/Procurement | document terms→receivable/payable | Parties | settlement; HIGH | M04/M05 / SPECIFIED CANDIDATE / domain specs / credit terms |
...
| Purchase order/receipt | Procurement/Inventory | demand→commitment/receipt | Parties, Catalog | replenishment; HIGH | M05 / SPECIFIED CANDIDATE / P2P spec / approvals/RFQ |
| Matching/payability | Procurement | PO+receipt/service+CPE→match/block | Tax, Documents | prevent wrong pay; HIGH | M05 / SPECIFIED CANDIDATE / tolerance spec / automation |
...
| Statement reconciliation | Treasury | statement+movements→matches/exceptions | import, Audit | daily control; CRITICAL | M06 / SPECIFIED CANDIDATE / bank spec / feeds |
| Corporate records | Corporate capability | acts/contracts→legal facts | Organization, Parties, Documents | governance; HIGH legal | M06 / SPECIFIED CANDIDATE / corporate spec / more shareholders |
...
| Accounting policies/posting | Accounting | source facts+policy→interpretations/asientos | domains, PCGE adapter | financial truth; CRITICAL | M07 / SPECIFIED CANDIDATE / policy catalog / new fact/framework |
| GL/subledgers/reconciliation | Accounting | postings→balances/control | policies, periods | explainability; CRITICAL | M07 / SPECIFIED CANDIDATE / R2R spec / scale |
| Close/adjust/reverse | Accounting | open balances→approved close | reconciliations | reliable period; CRITICAL | M07 / SPECIFIED CANDIDATE / close spec / maker-checker |
| NPIF package | Accounting | closed ledger→4 statements+notes | mappings, Documents | formal reporting; CRITICAL | M08 / SPECIFIED CANDIDATE / reporting spec / first real period |
| Framework transition | Accounting | prior framework→reconciled opening | normative register | growth; HIGH | M07/M08 / SPECIFIED CANDIDATE opening/transition; M10 / CONCEPT full destination framework / eligibility or formal choice |
| Tax profile/rules | Tax | entity+period+law→applicable policy | Research | compliance; CRITICAL | M09 / SPECIFIED CANDIDATE / RER policy / regime change |
| IGV/SIRE/reconciliation | Tax | CPE+facts+ledger→tax registers/positions | Sales, Procurement, Accounting | compliance; CRITICAL | M09 / SPECIFIED CANDIDATE / tax specs / obligation verified |
...
| CSV/Excel import | each owner | file→validate/preview/confirm | Documents, Audit | onboarding; HIGH bulk | M03/M05 / SPECIFIED CANDIDATE / per-domain schema / volume |
