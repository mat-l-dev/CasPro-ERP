...
Validación de extensibilidad de la enmienda de gobierno: nuevo nombre de documento para empresa/persona/equipo/vehículo/tarea con fuente, aplicabilidad, plantilla, vigencia, evidencia y estado ya representados = nueva revisión/configuración y maestros autorizados, **sin migración, módulo, campo hardcoded ni camino de código por nombre**. Dos minas seleccionan TemplateRevision distinta de un mismo formulario sin cambiar a la otra. ConfigDelegation para subfamilia de sitio más `sales.site.requirements.manage`/aprobación vigente y [manifest](../../architecture/configuration-governance.md) gobiernan el cambio; revalidar paquetes/visitas abiertos, conservar los enviados. Waiver cliente nunca dispensa LAW/REGULATION aplicables. Sujeto, efecto económico, workflow o autoridad legal genuinamente nuevos exigen spec acotada; no resolverlos con un campo arbitrario o script de plantilla.
