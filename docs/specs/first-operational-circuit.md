...
|---|---|---|
| HP1 | **CLOSED por el propietario:** venta B2C al contado; cobertura íntegra del objetivo vigente antes de cualquier despacho, aunque la entrega sea parcial. Precios/descuentos/envío requieren desglose autorizado y no hay tolerancia silenciosa | Falta concretar reglas de asignación del desglose en su spec si el canal no lo expresa; eso no habilita pago proporcional ni crédito |
| HP2 | **CLOSED:** PAID/refund de Jumpseller es observación; Treasury confirma dinero con cuenta, fecha, importe, moneda, referencia/evidencia suficiente y acto humano inicial | Formatos de cuenta/extracto y parametrización de evidencia se fijan con datos operativos sin exponer secretos; no inventar tránsito, saldo o liquidación de pasarela |
| HP3 | **PARTIALLY CLOSED:** promedio ponderado móvil para kardex operativo; coste no sustentado `UNKNOWN`; devolución de cliente inicialmente no vendible. VNR/deterioro/cierre pertenecen a Accounting | Posiciones elegibles, buffer/publicación y parámetros de precisión conservan gate técnico; apertura/ajuste real requieren sustento |
| HP4 | **PARTIALLY CLOSED:** CPE externo, vínculo inequívoco, preservación de originales y `AUTO_WITH_APPROVAL`; CasPro no reconstruye un XML como original | Normativa exacta, momento, identidad y artefactos obligatorios por finalidad requieren validación profesional; su ausencia bloquea la acción afectada |
| HP5 | **PARTIALLY CLOSED:** sin auto-delete empresarial inicial; recuperación conjunta DB+objetos; autoaprobación inicial transparente y maker/checker futuro | RPO/RTO, retención exacta, presupuesto/región y acciones que exigen aprobación adicional siguen pendientes antes de operación |

Las políticas tienen propietario, versión, vigencia y actor de aprobación: HP1 en Sales; HP2 en Treasury; HP3 en Inventory y Accounting según la separación anterior; HP4 en Sales y política de entrega en Documents; HP5 en cada dueño y operación. CLOSED describe una decisión empresarial de esta spec, no evidencia ejecutable ni una `TAX_RULE`. No tabla global de parámetros arbitrarios.
