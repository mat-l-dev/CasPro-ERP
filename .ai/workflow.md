...
Secuencia de cierre: REQUEST CHANGES → corrección acotada → re-revisión independiente del delta y consumidores afectados → aceptación del candidato exacto → sincronización de aceptación en review/PR → merge explícitamente autorizado → limpieza de rama integrada cuando esté autorizada y no contenga trabajo ajeno → siguiente fase autorizada. Conservar los REQUEST CHANGES y PASS de sus candidatos como historia. Un commit de estado identifica el candidato aceptado y su propio delta editorial; no se atribuye otra revisión semántica. Si la sincronización cambia comportamiento, deja de ser editorial y requiere revisión del nuevo delta. PASS del autor no es PASS independiente; este no autoriza merge y ninguno habilita implementación.
